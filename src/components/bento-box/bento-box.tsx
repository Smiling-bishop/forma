...
function classNamesFromSize(
    ...
    return cn(
        ...
        Object.entries(size)
            .map(([bp, sizeEl]) =>
                [bp !== "base" ? `${prefixBp}${bp}` : null, [cls.prefix, sizeEl, cls.suffix].join("")]
                    .filter(Boolean)
                    .join(":"),
            )
            .join(" "),
...
function bentoColsSize(cls: { prefix?: string; suffix?: string }, size: Responsive) {
    return cn(
        typeof size === "number" && [cls.prefix, size, cls.suffix].join(""),
        typeof size !== "number" &&
        Object.entries(size)
            .map(([bp, sizeEl]) =>
                [cls.prefix, bp !== "base" ? bp : null, sizeEl, cls.suffix].filter(Boolean).join("-"),
            )
            .join(" "),
    );
}
...
function BentoElement({ colSpan = 1, rowSpan = 1, layoutId, children }: BentoElementProps) {
